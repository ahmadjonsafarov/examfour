import 'package:examfour/view/pages/LoginAndRegister/signup/password_and_bio.dart';
...
class VerfyPage extends StatefulWidget {
  ...
  const VerfyPage({Key? key, required this.name, required this.usernme}) : super(key: key);
                ...
                GestureDetector(
                    onTap: () {
                      context.read<AuthController>().checkCode(controller.text,(){
                        Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_)=> Password(name: widget.name, username: widget.usernme,)), (route) => false);
                      });
                    },
                    child: Container(
